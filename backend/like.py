...
def likeTour(tour_id):
  ...
  if tour :
    new_card = db.card.update_one({"_id": ObjectId(tour_id)}, {"$set": {"like": like_count + 1} })
    db.like.insert_one({"user_id": payload["user_id"], "card_id": ObjectId(tour_id)})
...
def dislikeTour(tour_id):
  ...
  if tour :
    db.card.update_one({"_id": ObjectId(tour_id)}, {"$set": {"like": like_count - 1} })
    db.like.delete_one({"user_id": payload["user_id"], "card_id": ObjectId(tour_id)})
